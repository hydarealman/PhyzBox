...
窗口标题显示 UTC 日期、暂停 / 播放状态、当前倍率与色彩模式。画面底部只有一个半透明的时间控制条：可直接拖动九档流速，并持续显示当前倍率和“自动 / 手动”状态；播放按钮和 `Space`、拖动条和 `+ / -` 始终双向同步。玩家主动拖动或按 `+ / -` 后立即采用手动流速，避免接近历史事件时被自动减速悄悄覆盖；需要恢复飞掠保护时按 `A`。默认“任务可视化”模式采用 Filmic 曝光、深海军蓝天空和可辨识的行星色域；按 `C` 切换到较低曝光的物理观察模式。两种模式只改变渲染，不改变星历或物理状态。
