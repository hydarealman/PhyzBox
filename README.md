...
HUD 中的能量漂移、角动量漂移、最近距离都来自实时计算，可用于判断模拟是否仍然守恒得足够好。屏幕上的球体半径为了可视化被放大了，不代表真实恒星半径。
...
支持字段包括 `name`、`mass`、`radius`、`color`、`position`、`velocity`。单位仍然是 AU、太阳质量、年；速度单位是 AU/year。
